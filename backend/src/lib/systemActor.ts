 ...
 */
let cachedSystemActorId: string | null | undefined; // undefined = belum pernah dicari
...
export async function getSystemActorId(): Promise<string | null> {
  ...
  const resolvedId: string | null = superAdmin ? superAdmin.id : null;
  cachedSystemActorId = resolvedId;

  if (!cachedSystemActorId) {
    console.warn(
      '[systemActor] Tidak ada akun super_admin ditemukan — perubahan status otomatis ' +
