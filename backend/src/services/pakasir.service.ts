...
export function mapToPakasirMethod(method: string): string {
  return PAYMENT_METHOD_MAP[method.toLowerCase()] || 'qris';
}

export interface CreatePakasirPaymentParams {
  bookingId: string;
  amount: number;
...
export interface CreatePakasirPaymentResult {
  success: boolean;
  paymentUrl?: string;
  invoiceId?: string;
 ...
 */
export function calculatePakasirFee(amount: number, method: string): { fee: number; totalPayment: number } {
  let fee = 0;
 ...
 */
function generateSandboxVANumber(method: string): string {
...
export async function createPakasirPayment(
  params: CreatePakasirPaymentParams
): Promise<CreatePakasirPaymentResult> {
...
export interface PakasirPaymentStatus {
  success: boolean;
  status?: 'pending' | 'paid' | 'completed' | 'expired' | 'failed' | 'cancelled';
  invoiceId?: string;
  amount?: number;
  totalPayment?: number;
  fee?: number;
 ...
 */
export async function getPakasirPaymentStatus(
  invoiceId: string,
  amount?: number
): Promise<PakasirPaymentStatus> {
  if (!isPakasirConfigured()) {
    return { success: false, error: 'Pakasir belum dikonfigurasi' };
  }
  ...
  try {
    const url = `${PAKASIR_BASE_URL}/transactiondetail?project=${encodeURIComponent(
      PAKASIR_SLUG
    )}&amount=${baseAmount}&order_id=${encodeURIComponent(invoiceId)}&api_key=${encodeURIComponent(
      PAKASIR_API_KEY
    )}`;
    ...
    const returnedTotal =
      data.transaction?.total_payment ||
    ...
    return {
      success: true,
      status: (rawStatus?.toLowerCase() as PakasirPaymentStatus['status']) || 'pending',
 ...
 */
export function verifyPakasirWebhook(body: Record<string, unknown>): boolean {
 ...
 */
export function mapPakasirStatusToPayment(status: string): 'pending' | 'paid' | 'expired' | 'failed' {
 ...
 */
export function getPakasirPaymentUrl(invoiceId: string, amount?: number): string {
  if (amount) {
