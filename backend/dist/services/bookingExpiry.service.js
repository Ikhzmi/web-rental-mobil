...
async function expireStaleBookings() {
    ...
    try {
        ...
        const abandonedBookings = await prisma_1.prisma.booking.findMany({
            where: {
        ...
        const stalePendingBookings = await prisma_1.prisma.booking.findMany({
            where: {
                status: 'menunggu_pembayaran',
        ...
        for (const b of abandonedBookings) {
            await prisma_1.prisma.$transaction(async (tx) => {
                ...
                if (current?.status !== 'menunggu_pembayaran')
                    return; // sudah berubah status, lewati
 ...
 */
function startBookingExpiryJob(intervalMs = 10 * 60 * 1000) {
...
//# sourceMappingURL=bookingExpiry.service.js.map