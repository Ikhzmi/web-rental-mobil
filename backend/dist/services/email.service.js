...
// log ke console) agar server tetap berjalan normal di development
...
function formatTanggalEmail(iso) {
...
async function sendBookingEmail(subject, html, to) {
    if (!resend) {
...
async function sendBookingConfirmedEmail(data) {
    const html = wrapEmailLayout('Pesanan Kamu Dikonfirmasi ✓', `
...
async function sendBookingCancelledEmail(data, reason) {
    ...
    const html = wrapEmailLayout('Pesanan Dibatalkan', `
    <p style="font-size: 14px; line-height: 1.6;">Halo ${data.namaPenyewa},</p>
    <p style="font-size: 14px; line-height: 1.6;">
